...
class Training:
    ...
    LEN_STEP:float = 0.65 #длина расстояния при шаге (позже поменяем на длину расстояния при гребке)
...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        calories = (self.CALORIES_MEAN_SPEED_MULTIPLIER * self.get_mean_speed() + self.CALORIES_MEAN_SPEED_SHIFT) * self.weight / self.M_IN_KM * self.duration * self.H_IN_M 
        return calories
...
class SportsWalking(Training):
    ...
    def __init__(self,
                 ...
                 duration: float, #время, затраченное на тренировку
                 weight: float, #вес спортсмена
                 height: int, #рост спортсмена (в сантиметрах, дальше переведем в метры)
                 ) -> None:
    ...
    def get_spent_calories(self) -> float:
        ...
        calories = (self.CALORIES_WEIGHT_MULTIPLIER * self.weight + ((self.get_mean_speed() / self.KMPH_IN_MPS)**self.MEAN_SPEED_IN_MPS_EXP // (self.height / self.SM_IN_M)) * self.WEIGHT_MULTIPLIER * self.weight) * (self.duration * self.H_IN_M)
        return calories
...
class Swimming(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        spent_calories = (self.get_mean_speed() + self.CALORIES_MEAN_SPEED_MULTIPLIER) * self.MEAN_SPEED_CALORIES_MULTIPLIER * self.weight * self.duration
